...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, 
        batch_size=96,               # Increased for high VRAM capacity
        shuffle=True,
        num_workers=8,               # Optimal for RTX 6000 Ada
    ...
    if os.path.exists(val_annotations_file):
        ...
        val_loader = torch.utils.data.DataLoader(
            val_dataset, 
            batch_size=128,              # Larger for validation
            shuffle=False,
            num_workers=6,               # Slightly fewer than training
            collate_fn=collate_fn,
            persistent_workers=True,
            pin_memory=True
        )
    ...
    if os.path.exists(test_annotations_file):
        ...
        test_loader = torch.utils.data.DataLoader(
            test_dataset, 
            batch_size=192,              # Very large for inference
            shuffle=False,
            num_workers=12,              # More for test-only operations
            collate_fn=collate_fn,
            pin_memory=True
        )
